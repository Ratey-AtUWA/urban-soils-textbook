...
# start plotting!
par(mfrow=c(1,1), mar=c(3.5,3.5,0.75,0.75), mgp=c(1.8,0.3,0), 
...
bars <- Values
bars[3,] <- Values[1,] - Values[2,]
bars[2,] <- Values[2,] - Values[3,]
bars[1,] <- Values[3,]
bars <- as.matrix(bars)
barplot(bars, col=c("chocolate3","aquamarine3","navy"), xlab="", yaxt="n", 
    ylab=expression(paste("Volumetric water content, ",theta[v]," (v/v)")),
        ylim=c(0,0.59), yaxs="i", density=c(12,NA,18), angle=c(135,NA,45),
        names.arg=c("\u00A0\nSand\n\u00A0","\u00A0\nLoam\n\u00A0",
                    "\u00A0\nSilt\nloam","\u00A0\nSandy\nclay",
                    "Clay\n\u00A0","\u00A0\nCompac-\nted clay"),
        mgp=c(1.8,1.3,0))
axis(2, mgp=c(1.8,0.2,0))
legend(3.4,0.63, ncol=1, density=c(18,NA,12), angle=c(45,NA,135),
       legend=c("____",
                "____",
                "____"), 
       fill=c("navy","aquamarine3","chocolate3"),
       cex=2.4, bty="n", inset=0.01, x.intersp=0.5, 
       text.col="transparent")
legend(4.4,0.615, ncol=1, 
       legend=c("Drainage\n(\u2265 \u221210kPa)",
                "Available\n(\u221210 to \u22121500kPa)",
                "Unavailable\n(\u2264 \u22121500kPa)"), 
       col=c("transparent","transparent","transparent"),
       cex=1, bty="n", inset=0.01, x.intersp=0.1, y.intersp=1.6)
box()
#
...
legend("bottomleft", bty="n", cex=1.2, seg.len=1.2, 
       legend=c("Sand","Loam","Silt loam","Sandy clay",
                "Clay","Compacted clay"), 
       col=c(selected,"gray50"), pch=c(selected+12,NA), 
       x.intersp=0.5, 
       lty=c(rep(NA,length(selected)),1), 
       lwd=c(rep(1,length(selected)),3))
rm(list=c("bars","i","Values"))
