...
# beep(sound = 3)
# world_rob <- openproj(world.osm, projection=
...
# plot the map ####
png(file = "world cities gt_1e6 robin.png", height = 1826, width = 3840)
par(mar = c(1,1,1,1), mgp = c(2, 0.3, 0), tcl = 1, lend="square", 
    font.lab = 2)
plot(world_rob)
points(cities$x_robinson, cities$y_robinson, col = "darkred", pch=19, cex=0.5)
...
text(6e6, -1e6, labels = c("Population"), pos = 4, col = "black", 
     font = 4, cex = 4)
symbols(c(6e6, 6e6), c(-2e6, -3e6), circles = 100 * sqrt(c(1e7, 1e6)), 
        add=TRUE, inches = FALSE, fg = "red3", bg = "#C0000040", lwd = 3)
text(c(6e6, 6e6), c(-2e6, -3e6), 
