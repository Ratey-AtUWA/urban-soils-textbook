...
maxreg <- lm(As.log ~ pH+ EC.log+ Ca.log+ Fe.log+ K.log+
                 Na.log+ P.log+ S.log) # Al.log+ Mn.log+ 
summary(maxreg)
...
abline(c(-0.6206,1.3762,-1.5435,-3.2123,2.8508, 0.11), c(0.4775,-0.4193,0.8936,1.5323,-0.9588,0.0742))
abline(c(1.3762,-1.5435,-3.2123,2.8508, 0.11), c(-0.4193,0.8936,1.5323,-0.9588,0.0742))
abline(c(-1.5435,-3.2123,2.8508, 0.11), c(0.8936,1.5323,-0.9588,0.0742))
abline(c(-3.2123,2.8508, 0.11), c(1.5323,-0.9588,0.0742))
abline(c(2.8508, 0.11), c(-0.9588,0.0742))
abline(c(0.11), c(0.0742))
anova(regAs_S, regAs_S_grp)

# grouped regression for a simple regression model As vs S
require(car)
sp(mregdata$As.log ~ mregdata$S.log | mregdata$Group, smooth=F, pch = c(15,1,2,3,18,6), reset.par = F)
abline(lm(mregdata$As.log ~ mregdata$S.log), lty = 2)
